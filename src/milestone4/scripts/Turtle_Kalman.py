...
XTruth = [] #Array to hold ground truth value of x coordinates
YTruth = [] #Array to hold ground truth value of y coordinates
ThetaTruth = [] #Array to hold ground truth value of theta coordinates
...
def callback2(Pose):
	...
	#Save the positions in an array
	position = [x, y, Theta] 
	XTruth.append(position[0])
	YTruth.append(position[1])
	ThetaTruth.append(position[2])
...
### Initialize ROS Node 
	...
	sub2 = rospy.Subscriber('/Ground_Truth', Pose, callback2) #Subscribe to the  topic "/odom" of type "Odometry"
	pub = rospy.Publisher('/cmd_vel', Twist, queue_size=10) #Identify the publisher "pub1" to publish on
	rospy.spin()
...
##Plotting of signals from sensor and noisy signals
	...
	line_4 = plt.plot(XTruth, 'm-', label='X-GroundTruth')
	plt.legend()
	...
	line_4 = plt.plot(YTruth, 'm-', label='Y-GroundTruth')
	plt.legend()
	...
	line_4 = plt.plot(ThetaTruth, 'm-', label='Theta-GroundTruth')
	plt.legend()
